...
| `robustness` | 記録済みトレードを引き直し、成績が運と区別できるか検定 | しない |
...
| `forward_stop.sh` | 前向き検証をまとめて止める（**強制終了はしない**） | グループ名（既定 `forward`）／停止理由 |
...
scripts/forward_stop.sh                        # まとめて止める（再開は start）
```
...
```bash
scripts/forward_stop.sh                    # 6本まとめて止める
scripts/forward_stop.sh forward "PC再起動のため"   # 停止理由を残す
scripts/forward_start.sh                   # 再開（STOP は起動時に自動で消える）
```

各プロセスの `state_dir` にキルスイッチ（`STOP` ファイル）を置き、**次のループ境界で**取引ループ自身に止まってもらう。未約定注文の取消、リスク状態の保存、切断まで済ませてから終了する。`poll_interval_seconds: 60` なので最大1分ほどかかる。スクリプトは全部止まるまで待って結果を出す。

**停止要求はプロセスの生存確認に依存させていない。** `pgrep` が取りこぼしても `STOP` さえ置いてあれば取引ループは必ず止まる。逆に生存確認を条件にすると、取りこぼした瞬間に「止めたつもりで動き続けている」状態になる。止まっているプロセスに `STOP` を置いても害はない（起動時に消える）。

`STOP` は停止後もファイルとして残る。`forward_start.sh`（`zerotrade run`）が起動時に消すので、手で消す必要はない。

**強制終了はしない。** どうしても残る場合は `SIGTERM` を送る。CLI がハンドラを入れてあるので、これも次のループ境界まで待ってから安全に閉じる。`SIGKILL`（`kill -9`）だけは避けること。リスク状態の保存を飛ばす。
...
```bash
zerotrade -c config/forward/btc.yaml stop --reason "この銘柄だけ様子を見る"
zerotrade -c config/forward/btc.yaml status     # 止まったか確認
```

**建玉は閉じない（意図的）。** 停止のたびに全決済すると、ネットワーク断で再起動しただけで意図しない損失確定が起きる。シャドー実行の建玉と残高は `state/forward/<銘柄>/shadow_state.json` に残り、再開時に読み直される。約定のたびに書いているので、落ちた場合でも直前の約定までは残る。

再開しても**トレード件数の集計は続きから**になる。記録はSQLiteに残っているので、止めた時間ぶん判定が先に延びるだけである。

ただし止めている間の相場は当然拾えない。**そこで出たはずのシグナルは永久に欠測**になり、埋める方法はない。長く止めるなら、事前登録に「いつからいつまで止めた」を書き足しておくこと。後から都合よく解釈しないための記録である。
...
```bash
zerotrade backtest --csv data/btc_h1.csv --robustness    # 検証と同時に
zerotrade robustness --db state/backtests/backtest-20260808-180408.db
zerotrade robustness                                     # 実運用の記録に対して
```

バックテストが返すのは**1本の経路**でしかない。同じ戦略でもトレードの並びが少し違えば最終損益も最大ドローダウンも変わる。1本だけを見て「勝てる」と言うのは、コインを10回投げて7回表が出たのを見て「表が出やすいコインだ」と言うのに近い。

やっていることは単純で、**確定した損益列から復元抽出で同じ件数を引き直し、それを2,000回繰り返す**。得られた分布の中で、実際に観測した成績がどこに位置するかを見る。

実際の出力の例（BTC/USDT 1時間足・1年・ドンチアン）:

```
結果: 損益 +424 (+4.24%) / 247件 / 勝率 36% / PF 1.05 / 最大DD -1,141
検定: 247件のトレードを 2,000回 引き直し
  1トレードあたり: +1.72（標準偏差 106.75）
  最終損益: 観測 +424 / 5% -2,297 / 中央 +277 / 95% +3,248
  最大DD: 観測 1,141 / 中央 1,590 / 95% 3,123
  資金を減らして終わる確率: 43.0%
  この優位性を偶然と区別するのに必要な件数: 約 14,870件（現在 247件）
  判定: 5パーセンタイルがマイナス側。運と区別がついていない
```

**+4.24% は勝ちに見えるが、同じ確率分布から 43% の確率で負けが出る。** 観測した最大DD 1,141 も運が良かっただけで、現実的な上限は 3,123 のほうである。資金計画は観測値ではなく95パーセンタイルで立てる。

「必要な件数」は `n ≈ (1.96σ/μ)²` の正規近似で、**前向き検証を何件で打ち切るかを勘ではなく数字で決める**ために出している。247件で 14,870件必要と出るなら、この戦略はそもそも件数で解決する話ではない。

判定の基準はひとつだけにしてある。**最終損益の5パーセンタイルがプラス側にない成績を「勝てる戦略を見つけた」と呼ばない。**

### この検定で分からないこと

* **相場の変化には答えない。** 過去のトレードの分布を使い回すだけなので、相場つきが変わって優位性が消える可能性は評価できない。
* **トレードの独立性を仮定している。** 連敗が連鎖する戦略や、相関の高い複数銘柄を同時に持つ運用では、実際のドローダウンはここで出る値より深くなる。
* 元のトレード件数が2桁前半なら、分布そのものが当てにならない。出てくる数字は目安以上のものではない。

---

## 約定品質を測る（滑りと MFE/MAE）

バックテストはコストを**仮定で置いている**。その仮定が実測に負けていないかは、成績そのものより先に見るべき数字になる。1トレードあたりの優位性が 5bp しかないなら、往復の滑りが 3bp あるだけで半分以上が消える。

記録しているのは2種類ある。

**滑り** — 発注を決めた時点で観測していた価格（`reference_price`）と、実際の約定価格の差。**符号は方向で揃えてある**（買いは高く買わされた分、売りは安く売らされた分が正）。揃えないと買いと売りを平均した瞬間に打ち消し合って「ほぼゼロ」に見える。銘柄をまたいで比べられるよう bp でも出す。

**MFE / MAE** — 建玉中に到達した最大の含み益と最大の含み損。確定損益だけを見ていると「エントリーが悪いのか、出口が悪いのか」が区別できない。同じ −1,000 の負けでも、一度も含み益にならなかったトレードと、含み益 +5,000 を取り逃してから負けたトレードでは直す場所が違う。

* 実現損益が MFE に遠く及ばない → **出口が悪い**
* 勝ちトレードの MAE が深い → ストップが広すぎるか、エントリーが早い

`zerotrade report` の「約定品質」と、`robustness` の末尾に出る。取り切り率は**比の平均ではなく合計どうしの比**で出している。1件ずつの比を平均すると、含み益がほぼ0のトレードで分母が潰れて外れ値ひとつで壊れるため。

測る粒度はブローカーによって違う。バックテストとペーパーは**足の高値・安値**で1本ずつ観測するので実質的な上限・下限を捉える。ライブはループごとの気配値を標本にするだけなので、ループ間の突発的な行き過ぎは取りこぼす。**過小評価する方向にしか外れない。**

記録層の列は起動時に自動で足される（`trades.mfe` / `trades.mae` / `orders.reference_price`）。稼働中のDBを作り直す必要はない。古い行の値は `None` のままで、集計からは除外される。**無いものを0として混ぜると平均が実態より良く見える。**
...
止め方は3つあり、**どれも次のループ境界まで待ってから安全に閉じる**。

| 方法 | 使う場面 |
|------|---------|
| `zerotrade -c <config> stop` | 通常。理由も記録に残る |
| `Ctrl-C` / `SIGTERM` | そのターミナルで前面実行しているとき |
| `scripts/forward_stop.sh` | 前向き検証をグループごとまとめて |

**`kill -9`（SIGKILL）だけは避けること。** リスク状態の保存と未約定注文の取消を飛ばす。次の起動で日次・週次のカウンタが古い状態から始まる。

停止時に**建玉は閉じない**。これは意図的で、一時的なネットワーク断で再起動しただけで意図しない損失確定が起きるのを避けるためである。建玉をどうするかは運用者の判断に残してある。
