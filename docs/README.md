...
| ドキュメント | 内容 |
|-------------|------|
| [tools.md](tools.md) | コマンド一覧、**スクリプト一覧**、設定、日々の運用、トラブルシューティング |
| [backtesting.md](backtesting.md) | 検証の進め方と、**自分を騙さないための8項目** |
| [strategies.md](strategies.md) | 同梱戦略の中身、実測成績、**新しい戦略の書き方と採用までの道のり** |
...
実際に発注するのは `run` と `verify` だけで、他のコマンドはすべて安全に試せる。
...
書き方から採用までの一連の流れは [strategies.md の「採用までの道のり」](strategies.md#書いたあと--採用までの道のり) にまとめてある。仮説 → バックテスト → ロバストネス検定 → 事前登録 → 前向き検証 → 実弾の各段階に関門があり、**1つでも欠けたら先へ進まない**。

**実弾を入れる前には [backtesting.md の「自分を騙さないための8項目」](backtesting.md#自分を騙さないための8項目) を必ず読むこと。** このリポジトリで実際に起きた失敗（開発用データで +18.8% を出した戦略が検定用データで -7.4% に転落した件）が、なぜ起きたかも含めて書いてある。
